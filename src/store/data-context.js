...
export const DataContextProvider = (props) => {
  ...
  const PlusSizeHandler = () => {
    if (PassElemUp) {
      const PlusSizeChangerUp =
        document.getElementById(ElemidUp).style.fontSize;
      let PlusFinalSizeUp = parseInt(PlusSizeChangerUp, 10);
      setSizeUpp(PlusFinalSizeUp + 5);
    }
    if (PassElemLo) {
      const PlusSizeChangerLo =
        document.getElementById(ElemidLo).style.fontSize;
      let PlusFinalSizeLo = parseInt(PlusSizeChangerLo, 10);
      setSizeLow(PlusFinalSizeLo + 5);
    }
    if (PassElemTh) {
      const PlusSizeChangerTh =
        document.getElementById(ElemidTh).style.fontSize;
